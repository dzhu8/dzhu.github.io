...
function updateCranePositionsAndReferences(deltaTime) {
     ...
     for (let i = 0; i < craneInstances.length; i++) {
          ...
          // Calculate overall progress along the entire path
          const totalPathLength = params.pathPoints.length - 1;
          const overallProgress = (params.currentPathIndex + params.pathProgress) / totalPathLength;
...
function updateCraneScales() {
     ...
     craneInstances.forEach((craneInstance, index) => {
          if (craneInstance && craneInstance.craneObject) {
